...
import { ObjectifGridComponent } from './grids/objectif-grid.component';
 ...
 */
@Component({
  selector: 'app-crm-objectifs',
  standalone: true,
  imports: [DashboardHeaderComponent, ObjectifGridComponent, TranslateKeyPipe, TranslateDirective],
  templateUrl: './crm-objectifs.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  styleUrl: './crm-objectifs.component.scss'
})
export class CrmObjectifsComponent extends BaseComponent implements OnInit {
  private readonly _PlatformId: object = inject(PLATFORM_ID);
  private readonly _DestroyRef: DestroyRef = inject(DestroyRef);

  /** @inheritdoc */
  public ngOnInit(): void {
    // - cm - Charge le sous-registre crm.objectifs pour résoudre le titre/header
    // et les colonnes/filtres de la grid (sinon la grid affiche les clés i18n brutes).
    if (isPlatformBrowser(this._PlatformId)) {
      void this.Translate.loadPageTranslations('crm.objectifs');
    }
    ...
    this.Translate.LanguageChanged
      ...
      .subscribe((pLang: SupportedLanguage) => {
        void this.Translate.loadPageTranslations('crm.objectifs', pLang);
      });
  }
}
